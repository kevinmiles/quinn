...
use quinn::{CertificateChain, PrivateKey, ServerConfig};
...
fn run(options: Opt) -> Result<()> {
    let config = configure_server(&options)?;
...
async fn handle_request(
    ...
    let response = process_get(&root, &request).unwrap_or_else(|e| {
        error!("failed: {}", e);
...
fn configure_server(options: &Opt) -> Result<ServerConfig> {
    ...
    let (key, certificate_chain) =
        if let (Some(key_path), Some(cert_path)) = (&options.key, &options.cert) {
            create_certificate(key_path, cert_path)?
        } else {
            create_self_signed_certificate()?
        };

    server_config.certificate(certificate_chain, key)?;
    Ok(server_config.build())
}

fn create_self_signed_certificate() -> Result<(PrivateKey, CertificateChain)> {
    ...
    Ok((key, quinn::CertificateChain::from_certs(cert)))
}

fn create_certificate(
    key_path: &PathBuf,
    cert_path: &PathBuf,
) -> Result<(PrivateKey, CertificateChain)> {
